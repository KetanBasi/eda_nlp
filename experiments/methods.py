...
def get_txt_paths(folder):
    txt_paths = [
        ...
        if isfile(join(folder, f)) and ".txt" in f
    ]
...
def get_x_y(
    train_txt, num_classes, word2vec_len, input_size, word2vec, percent_dataset
):
    # read in lines
...
def build_model(sentence_length, word2vec_len, num_classes):
    ...
    model.add(
        Bidirectional(
            LSTM(64, return_sequences=True), input_shape=(sentence_length, word2vec_len)
        )
    )
...
def build_cnn(sentence_length, word2vec_len, num_classes):
    ...
    model.add(
        layers.Conv1D(
            128, 5, activation="relu", input_shape=(sentence_length, word2vec_len)
        )
    )
